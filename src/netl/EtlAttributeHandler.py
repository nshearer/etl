...
class FrozenAttributeCollection(FrozenAttributeValue):
    ...
    @abstractmethod
    def restore(self, value, meta):
        # TODO: Handle restoring NotFrozenAttributeValue
        if meta['type'] == 'single':
            return FrozenAttributeValue(value, meta['orig_class'])
        elif meta['type'] == 'collection':
            return FrozenAttributeCollection(value, meta['orig_class'], meta['item_metadata'])
        else:
            raise Exception("Unknown value type: " + meta['type'])
...
class EtlAttributeHandler:
    ...
    def store(self, value):
        ...
        def _convert_frozendict(v):
            '''Remove frozen dict from metadata dicts so JSON can serialize'''
            if v.__class__ is frozendict:
                v = dict(v)
            if v.__class__ is dict:
                for k, i in v.items():
                    v[k] = _convert_frozendict(i)
            return v

        meta = _convert_frozendict(value.freeze_collection_metadata())
    ...
    def prepickle_frozendict(self, value, items):
    ...
    def restore(self, data):
        '''
        Generate attribute value representation from a string generated by store()

        string -> FrozenAttributeValue

        :param value: value from store()
        :return: FrozenAttributeValue containing value to be stored
        '''


        def _convert_frozendict(v):
            '''Make sure all dictionaries are frozendicts'''
            if v.__class__ is dict:
                return frozendict({k: _convert_frozendict(v) for (k, v) in v.items()})
            else:
                return v
        ...
        value = meta['value']
        del meta['value']
        value = self._postpickle_raw_value(value)

        meta = _convert_frozendict(meta)

        return FrozenAttributeCollection.restore(value, meta)


    def _postpickle_raw_value(self, value):
        '''
        postpickle -> reverse the effects of prepickle

        :param value: Raw value that was stored from prepickle()
        :return: raw value stored in value
        '''
        ...
        # Find postpickle method
        try:
            postpickle_calc_name = 'postpickle_%s' % (self._calc_value_clsname(value))
            postpickle_calc = getattr(self, postpickle_calc_name)
        except AttributeError:
            msg = "Missing session.attribute_handler.%s())" % (postpickle_calc_name)
            raise NoAttributeValueHandler(msg)

        # For collection, recursivly postpickle
        try:
            postpickeled_items = dict()
            for key, item in self._list_collection_members(value):
                postpickeled_items[key] = self._postpickle_raw_value(item)

            # Post-Pickle collection
            return postpickle_calc(value, postpickeled_items)

        # For non-collections, just pre-pickle
        except AttrNotACollection:
            return postpickle_calc(value)


    def postpickle_date(self, value):
        return value.strftime(self.DATE_FORMAT)

    def postpickle_datetime(self, value):
        return value.strftime(self.DATE_FORMAT)

    def postpickle_decimal(self, value):
        return str(value)

    def postpickle_tuple(self, value, items):
        return [items[i] for i in sorted(items.keys())]

    def postpickle_frozendict(self, value, items):
        return {k: v for (k, v) in items.items()}
